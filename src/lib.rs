...
impl Guest for MetaComponent {
    fn page(edgee_event: Event, settings: Dict) -> Result<EdgeeRequest, String> {
        if let Data::Page(ref data) = edgee_event.data {
            ...
            if meta_payload.data_collection_method != "edge" {
                return Err("Data collection method is not supported".to_string());
            }
...
fn build_edgee_request(meta_payload: MetaPayload) -> EdgeeRequest {
    ...
    let domain = match meta_payload.servers_location.as_str() {
        "EU" => "sgw-eu.edgee.app",
        "US" => "sgw-us.edgee.app",
        _ => "sgw-eu.edgee.app",
    };
...
mod tests {
    ...
    fn sample_user_data_invalid_without_email_or_phone() -> UserData {
        UserData {
            user_id: "123".to_string(),
            anonymous_id: "456".to_string(),
            edgee_id: "abc".to_string(),
    ...
    fn sample_user_event_without_email_or_phone(
        consent: Option<Consent>,
        edgee_id: String,
        locale: String,
        session_start: bool,
    ) -> Event {
        let user_data = sample_user_data_invalid_without_email_or_phone();
        Event {
            uuid: Uuid::new_v4().to_string(),
            timestamp: 123,
            timestamp_millis: 123,
    ...
    fn user_event_without_email_or_phone_fails() {
        let event = sample_user_event_without_email_or_phone(
            Some(Consent::Granted),
            "abc".to_string(),
            "fr".to_string(),
            true,
        ...
        assert_eq!(
            result
                ...
                .unwrap()
                .to_string()
                .contains("User properties must contain email or phone_number"),
            true
        );
    }
